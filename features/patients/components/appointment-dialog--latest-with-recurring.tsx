...
export function AppointmentDialog({ isOpen, onClose, patientId, onSuccess, currentAppointment }: AppointmentDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && currentAppointment) {
      ...
      form.setValue("isRecurring", false); // Editing doesn't support recurring
  ...
  const addInterval = (date: Date, every: number, frequency: string) => {
    ...
    switch (frequency) {
      ...
      case "MONTHLY":
        ...
        break;
      case "YEARLY":
        newDate.setFullYear(newDate.getFullYear() + every);
        break;
      default:
        break;
    }
  ...
  const onSubmit = async (values: AppointmentForm) => {
    ...
    try {
      ...
      if (
        ...
        // Recurring logic
        const every = values.recurringInterval || 1;
        const frequency = values.recurringType;
        const recurringEndDate = values.recurringEndDate;
        let appointments = [];
        ...
        while (currentStart <= recurringEndDate) {
          ...
          currentStart = addInterval(currentStart, every, frequency);
          currentEnd = addInterval(currentEnd, every, frequency);
        }
        let allSuccess = true;
        let firstError = null;
        let successCount = 0;
        for (const appt of appointments) {
          const res = await createAppointment(appt);
          if (!res.success) {
            allSuccess = false;
            if (!firstError) firstError = res.error || "Failed to save appointment";
          } else {
            successCount++;
          }
        }
        if (allSuccess) {
          toast.success(`${successCount} recurring appointments scheduled successfully.`);
        } else {
          toast.error(firstError || "Some appointments failed to save");
        }
      } else {
        // Single appointment
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Schedule multiple appointments automatically
                    </div>
                  </div>
                  <FormControl>
                    <Switch
            ...
            {isRecurring && (
              <div className="mt-2">
                <Label className="block mb-1">Recurring Appointment</Label>
                <div className="grid grid-cols-3 gap-4 w-full">
                  {/* Every */}
                  <div className="flex flex-col w-full">
                    <Label htmlFor="recurring-every" className="mb-1">Every</Label>
                    <Input
                      id="recurring-every"
                      type="number"
                      min={1}
                      value={form.watch("recurringInterval")}
                      onChange={e => form.setValue("recurringInterval", Number(e.target.value) || 1)}
                      className="w-full"
                    />
                  </div>
                  {/* Frequency */}
                  <div className="flex flex-col w-full">
                    <Label htmlFor="recurring-frequency" className="mb-1">Frequency</Label>
                    <Select value={form.watch("recurringType")} onValueChange={val => form.setValue("recurringType", val)}>
                      <SelectTrigger className="w-full">
                  ...
                  {/* End Date (treatment dialog style) */}
                  <div className="flex flex-col w-full">
                    <Label htmlFor="recurring-end-date" className="mb-1">End Date</Label>
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                        ...
                        >
                          {form.watch("recurringEndDate")
                            ? new Date(form.watch("recurringEndDate")).toLocaleDateString()
                            : "Select end date"}
                          <ChevronDownIcon />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          mode="single"
                          selected={form.watch("recurringEndDate") ? new Date(form.watch("recurringEndDate")) : undefined}
                          captionLayout="dropdown"
                          onSelect={date => form.setValue("recurringEndDate", date ? date.toISOString().slice(0, 10) : "")}
                          disabled={date => date < new Date(new Date().setHours(0, 0, 0, 0))}
                        />
                      </PopoverContent>
                    </Popover>
